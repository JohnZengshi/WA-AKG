...
import { roomSupportsDuplicateStatus } from "@/server/socket";
    ...
    async handleConnectionUpdate(update: Partial<ConnectionState>) {
        ...
        try {
                ...
                if (shouldReconnect) {
                    ...
                    try {
                        await prisma.$transaction([
                            prisma.session.update({
                                where: { sessionId: this.sessionId },
                                data: { status: "LOGGED_OUT", qr: null, waJid: null }
                            }),
                            prisma.authState.deleteMany({
                                where: { sessionId: this.sessionId }
                            })
            ...
            if (connection === "open") {
                ...
                const waJid = rawJid.replace(/:\d+@/, "@") || null;

                // --- duplicate WhatsApp account check ---
                if (waJid) {
                    const duplicate = await prisma.session.findFirst({
                        where: { waJid, sessionId: { not: this.sessionId } },
                ...
                try {
                    await prisma.session.update({
                        where: { sessionId: this.sessionId },
                        data: { status: "CONNECTED", qr: null, waJid }
                    });
                } catch (e: any) {
                    if (e.code === "P2002") {
                        // Race: another session claimed this waJid first
